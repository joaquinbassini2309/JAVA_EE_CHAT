...
const app = createApp({
  setup() {
    ...
    const fetchConversations = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filtrar conversaciones que no tengan mensajes (estilo WhatsApp)
          conversations.value = allConvs.filter(c => c.ultimoMensaje != null);
        }
      } catch (error) {
    ...
    const connectWebSocket = (convId) => {
      ...
      const wsUrl = `${wsProtocol}//${window.location.host}/chat-empresarial/ws/conversacion/${convId}/usuario/${usuario.id}?token=${token}`;
      ...
      ws.onmessage = (event) => {
        ...
        if (data.tipo === 'mensaje') {
          // 1. Si es de la conversación activa, añadir al chat
          if (data.datos.conversacionId === activeConversation.id) {
            activeConversation.mensajes.push(data.datos);
            scrollToBottom();
          }
          
          // 2. Actualizar último mensaje en la barra lateral
          const conv = conversations.value.find(c => c.id === data.datos.conversacionId);
          if (conv) {
            conv.ultimoMensaje = data.datos.contenido;
          } else {
            // Si el chat era nuevo/vacío y no estaba en la lista, recargar lista
            fetchConversations();
          }
        }
      };

      ws.onopen = () => console.log('WebSocket conectado a conv:', convId);
                  ...
                  <div class="sidebar-list-scroll">
                    <v-list density="comfortable" lines="two" class="bg-transparent py-2">
                      ...
                      >
                        ...
                        <v-list-item-subtitle>{{ conv.ultimoMensaje || 'Sin mensajes' }}</v-list-item-subtitle>
                      </v-list-item>
                    </v-list>
                  </div>
                </v-col>
