<#
run.ps1 - Script de arranque local para Windows (PowerShell)
Basado en el `run.sh` que adjuntaste. Hace:
  - limpia puertos 8080/9990
  - compila frontend con npm (CapaPresentacion/Web) y copia dist a CapaServicio/src/main/webapp/
  - compila el WAR con Maven (CapaServicio)
  - copia el WAR al directorio de despliegue de WildFly
  - reinicia/arranca un contenedor Docker de Postgres (nombre configurable)
  - lanza WildFly (standalone.bat) y espera a que presiones Enter para detener todo
...
if (-not (Test-Path $wildFlyBin)) {
    ...
    Write-Error-Custom "Edita el script o ejecuta con: .\run.ps1 -WildFlyDir 'C:\ruta\a\wildfly-32.0.1.Final'"
}
...
try {
    ...
    Invoke-Step "Compilando Frontend con npm run build..." {
        # Use the npm executable that is resolved in PATH. On Windows it's usually npm.cmd
    ...
    Invoke-Step "Compilando proyecto con Maven..." {
        mvn -B -pl CapaServicio -am -DskipTests package
...
try {
    if (Test-Path $wildFlyBin) {
        ...
        Write-Warning-Custom "No puedo iniciar WildFly porque no encontré $wildFlyBin. Saltando el arranque."
    }
} catch {
...
if (Test-Path $wildFlyBin) {
    ...
    Write-Info "App: (WildFly no iniciado) revisa la variable -WildFlyDir"
}
...
if ($wfProcess -and -not $wfProcess.HasExited) {
    try { Stop-Process -Id $wfProcess.Id -Force -ErrorAction SilentlyContinue } catch {}
}
